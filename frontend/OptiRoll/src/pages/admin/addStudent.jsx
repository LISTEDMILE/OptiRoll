...
export default function AdminAddStudent() {
  ...
  const [selectedDeviceId, setSelectedDeviceId] = useState(null); // deviceId or null
  ...
  // enumerate devices and auto-select known external webcams
useEffect(() => {
  let mounted = true;

  async function fetchDevices() {
    try {
      // Temporary stream to get permissions
      let stream;
      try {
      ...
      if (mounted) {
        setVideoDevices(vids); // show all cameras
        // Keep selectedDeviceId null to use default facingMode
      }
          ...
          <div className="rounded-3xl border border-white/10 bg-white/10 p-6 shadow-2xl shadow-black/40 backdrop-blur-xl md:p-8">
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input type="text" name="rollNo" value={form.rollNo} onChange={onChange} placeholder="123456" className="w-full rounded-2xl border border-white/15 bg-white/5 px-4 py-3 text-white placeholder-white/40 outline-none transition focus:border-fuchsia-400/60 focus:bg-white/10" />
              </div>
