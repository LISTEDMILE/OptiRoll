            ...
            if (req.files && req.files.images && req.files.images.length > 0) {
                for (const file of req.files.images) {
                    try {
                        ...
                        const encoding = await getFaceEncodingFromBuffer(
                            file.buffer,
                        );
                        ...
                        return res.status(400).json({
                            errors: ["No face detected in some of images"],
                            oldInputs: {
                                name,
                                email,
                                rollNumber,
                ...
                if (encodings.length > 1) {
                    const avgEncoding = encodings[0].map((_, i) => {
                        return (
                            encodings.reduce((sum, enc) => sum + enc[i], 0) /
                            encodings.length
                        );
                    });
...
exports.adminStudentList = async (req, res, next) => {
    try {
        ...
        await adminUser.populate({
            path: "students",
            select: "_id name email rollNumber phone profilePicture course",
        });

        const studentsList = adminUser.students;
...
exports.adminStudentDashboard = async (req, res, next) => {
    try {
        ...
        const student = await StudentUser.findById(sid).select(
            "_id name address emergencyContact email password rollNumber dateOfBirth gender phone profilePicture course year section parentName parentPhone parentEmail hobbies bio skills achievements",
        );
        ...
        try {
            ...
            if (req.body.address) {
                student.address = {
                    ...
                    country:
                        req.body.address.country || student.address?.country,
                };
            }
            ...
            return res
                .status(500)
                .json({ errors: ["Error editing dashboard"] });
        }
    },
];
...
exports.deleteStudent = async (req, res, next) => {
    try {
        ...
        const adminUser = await AdminUser.findById(req.session.AdminUser._id).select("students");
        if (!adminUser.students.includes(sid)) {
            return res.status(401).json({
                errors: ["Unauthorized Access"],
            });
...
exports.adminStudentAttencence = async (req, res, next) => {
    try {
        ...
        const adminUser = await AdminUser.findById(req.session.AdminUser._id).select("students attendence");
        if (!adminUser.students.includes(sid)) {
            return res.status(401).json({
                errors: ["Unauthorized Access"],
            });
        ...
        try {
            const student = await StudentUser.findById(sid).select("attendence");
            if (!student) {
                return res.status(404).json({
...
exports.adminAdminAttencence = async (req, res, next) => {
    try {
        ...
        try {
            const admin = await AdminUser.findById(sid).select("attendence");
            if (!admin) {
                return res.status(404).json({
