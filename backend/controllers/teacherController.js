...
exports.teacherMarkAttendence = async (req, res) => {
    try {
        if (
            !req.session ||
        ...
        for (const studentId of adminUserStudents.students) {
            const student = await StudentUser.findById(studentId).select(
                "_id name email faceEncoding attendence",
            );
            if (!student.faceEncoding?.length) continue;

            const distance = cosineDistance(
                uploadedEmbedding,
                student.faceEncoding[0],
            );
        ...
        if (studentUser.attendence.whatNext == "start") {
            ...
            if (formatDate(startTime) === formatDate(endTime)) {
                const existing = studentUser.attendence.data.find(
                    (atten) =>
                        formatDate(new Date(atten.Date)) ===
