    ...
    try {
        if (loginType === "student") {
            ...
            req.session.regenerate((err) => {
                ...
                req.session.StudentUser = { _id: studentUser._id };
            ...
            const adminUser = await AdminUser.findOne({ email: email }).select(
                "_id password",
            );
            if (!adminUser) {
                return res.status(401).json({
...
exports.postDeleteAccount = async (req, res, next) => {
    try {
        ...
        const students = await StudentUser.find({
            _id: { $in: adminUser.students },
        }).select("profilePicture");

        for (const student of students) {
            if (!student.profilePicture) continue;
            ...
            try {
                ...
                console.error(
                    `Error deleting profile picture for student ${student._id}:`,
                    err.message,
                );
                return res
                    .status(500)
                    .json({ errors: ["Error deleting profile picture."] });
            }
        }
